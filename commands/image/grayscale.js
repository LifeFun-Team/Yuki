...
const { grayscale } = require('../../util/canvas');
exports.run = async (bot, message, args) => {
  ...
  let URL = message.attachments.first() ? message.attachments.first().url : null || message.mentions.users.first() ? message.mentions.users.first().avatarURL({format: 'png', size: 2048}) : null|| message.author.avatarURL({format: 'png', size: 2048}) || message.id
      ...
      grayscale(ctx, 0, 0, data.width, data.height);
      const attachment = canvas.toBuffer();
      if(Buffer.byteLength(attachment) > 8e+6) return message.say('Изображение было выше 8 МБ.');
      return message.say({ files: [{ attachment, name: 'grayscale.png' }] });
  } catch(err) {
    message.say({ files: [{ attachment: "https://cdn.discordapp.com/attachments/623994687409225739/727564716788154438/nonegrayscale.png", name: 'grayscale.png' }] });
  }
};
exports.help = {
  name: 'grayscale',
  aliases: [],
  info: 'Переделывает аватар пользователя в черно белые тона',
